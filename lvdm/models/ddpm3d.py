...
from lvdm.models.modules.util import (
    ...
    extract_into_tensor,
    noise_like,
    random_temporal_masking,
)
from lvdm.samplers.ddim import DDIMSampler
from lvdm.utils.common_utils import (
    exists,
    default,
    ismap,
    isimage,
...
from lvdm.models.modules.condition_modules import CLIPVisionEmbedder
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # print("aaaaaaaaaaaaaaaaaaaaaaaaaa", image_size)
        if isinstance(self.image_size, int):
            self.image_size = [self.image_size, self.image_size]
        self.channels = channels
        self.model = DiffusionWrapper(unet_config, conditioning_key)
        self.conditioning_key = (
            conditioning_key  # also register conditioning_key in diffusion
        )
        ...
        self.temporal_length = (
            ...
            else unet_config.params.temporal_length
        )
        self.total_length = (
            video_length
            if video_length is not None
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        for k in keys:
            for ik in ignore_keys:
                if k.startswith(ik) or (
                    ik.startswith("**") and ik.split("**")[-1] in k
                ):
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        ...
        return self.p_sample_loop(size, return_intermediates=return_intermediates)
    ...
    def training_epoch_end(self, outputs):
        ...
        self.log(
            "train/epoch/time", self.current_epoch_time, logger=True, on_epoch=True
        )
        self.log(
            "train/epoch/time_avg", self.epoch_time_avg, logger=True, on_epoch=True
        )
        self.log(
            "train/epoch/time_avg_min",
            self.epoch_time_avg / 60,
            logger=True,
            on_epoch=True,
        )
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.instantiate_img_cond_stage(img_cond_stage_config)
        self.cond_stage_forward = cond_stage_forward
        self.clip_denoised = False
        self.cond_stage_config = cond_stage_config
        self.first_stage_config = first_stage_config
        self.img_cond_stage_config = img_cond_stage_config
        ...
        # self.cond_stage_vision_model = CLIPVisionEmbedder()
    ...
    def instantiate_img_cond_stage(self, config):
        vision_model = instantiate_from_config(config)
        self.cond_stage_vision_model = vision_model
    ...
    def get_condition(self, batch, x, bs, force_c_encode, k, cond_key):
        ...
        if is_conditional:
            ...
            if self.cond_stage_config is not None and (
                not self.cond_stage_trainable or force_c_encode
            ):
    ...
    def get_input(
        ...
        first_frame_image = batch["first_frame"].cpu()
        if len(first_frame_image.shape) < 4:
            first_frame_image = first_frame_image.unsqueeze(0)
        first_frame_image = [f for f in first_frame_image]
        # first_frame = torch.swapaxes(first_frame, 1, -1).cpu()
        first_frame_context = self.cond_stage_vision_model.encode(
            images=first_frame_image
        )
        ...
        first_frame_encoded = z[:, :, 0]

        if self.latent_frame_strde:
            z = z[:, :, ::4, :, :]
            assert (
                z.shape[2] == self.temporal_length
            ), f"z={z.shape}, self.temporal_length={self.temporal_length}"
        ...
        # !!! Edited by me
        out.append(first_frame_encoded)
        out.append(first_frame_context)
    ...
    @torch.no_grad()
    def overlapped_decode(
        self, z, max_z_t=None, overlap_t=2, predict_cids=False, force_not_quantize=False
    ):
        if max_z_t is None:
            max_z_t = z.shape[2]
    ...
    def decode_first_stage_2DAE(self, z, decode_bs=16, return_cpu=True, **kwargs):
        ...
        if decode_bs is None:
            ...
            if return_cpu:
                results = torch.cat(
                    [self.decode(z_, **kwargs).cpu() for z_ in z], dim=0
                )
            else:
    ...
    def decode_first_stage(
        ...
        **kwargs,
        ...
        if self.encoder_type == "2d" and z.dim() == 5:
            return self.decode_first_stage_2DAE(
        ...
        if decode_single_video_allframes:
            ...
            (self.clip_length is not None)
            and (self.downfactor_t is not None)
            and (z.shape[2] > self.clip_length // self.downfactor_t)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        """
        x, c, first_frame_encoded, img_c = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, first_frame_encoded, img_c)
        return loss

    def forward(self, x, c, first_frame, img_c, *args, **kwargs):
        t = torch.randint(
            0, self.num_timesteps, (x.shape[0],), device=self.device
        ).long()
        ...
        return self.p_losses(x, c, t, first_frame, img_c, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, **kwargs):
        ...
        if "first_frame_encoded" in kwargs:
            x_noisy[:, :, 0] = kwargs["first_frame_encoded"]

        # ORIGINALLY:
        # x_recon = self.model(x_noisy, t, **cond, **kwargs)

        step = 4
        video_length = x_noisy.shape[2]
        x_recon = []
        for i in range(0, video_length, step):
            video_chunk = self.model(x_noisy[:,:,i:(i+step)], t, **cond, **kwargs)
            x_recon.append(video_chunk)

            x_noisy[:,:,i+step] = video_chunk[:,:,-1]
            first_frame_encoded = video_chunk[:,:,-1].unsqueeze(2)
            first_frame_image = self.decode_first_stage(first_frame_encoded)
            first_frame_image = [f.squeeze(1) for f in first_frame_image]
            kwargs["img_cond"] = self.cond_stage_vision_model.encode(
                images=first_frame_image
            )
        x_recon = torch.cat(x_recon, 2)
    ...
    def p_mean_variance(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            if isinstance(c, torch.Tensor):
                ...
                model_out_uncond, model_out = self.apply_model(x_in, t_in, c_in).chunk(
                    2
                )
                ...
                raise NotImplementedError
            if uc_type is None:
                model_out = model_out_uncond + unconditional_guidance_scale * (
                    model_out - model_out_uncond
                )
            else:
                if uc_type == "cfg_original":
                    model_out = model_out + unconditional_guidance_scale * (
                        model_out - model_out_uncond
                    )
                elif uc_type == "cfg_ours":
                    model_out = model_out + unconditional_guidance_scale * (
                        model_out_uncond - model_out
                    )
                else:
                    raise NotImplementedError
    ...
    def log_condition(self, log, batch, xc, x, c, cond_stage_key=None):
        ...
        if self.model.conditioning_key is not None:  # concat-time-mask
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning_txt_img"] = log_txt_as_img(
                    txt_img_shape, batch[cond_stage_key], size=x.shape[3] // 25
                )
                log["conditioning_txt"] = batch[cond_stage_key]
            elif cond_stage_key == "class_label":
                try:
                    xc = log_txt_as_img(
                        txt_img_shape, batch["human_label"], size=x.shape[3] // 25
                    )
                except:
                    xc = log_txt_as_img(
                        txt_img_shape, batch["class_name"], size=x.shape[3] // 25
                    )
                log["conditioning"] = xc
            elif isimage(xc):
                log["conditioning"] = xc
    ...
    def log_images(
        ...
        **kwargs,
        ...
        # get input
        z, c, x, xrec, xc, first_frame_encoded, img_c = self.get_input(
            batch,
        ...
        if sample:
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(
                    ...
                    eta=ddim_eta,
                    temporal_length=self.temporal_length,  # self.video_length,
                    unconditional_guidance_scale=unconditional_guidance_scale,
            ...
            x_samples = self.decode_first_stage(samples, log_images=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.conditioning_key is None:
            ...
            if "img_cond" in kwargs:
                img_c = kwargs["img_cond"]
                img_c_repeated = img_c.unsqueeze(1).repeat_interleave(85, 1)
                cc = torch.cat([cc, img_c_repeated], 1)
                out = self.diffusion_model(
                    x, t, context=cc, **kwargs
                )
            else:
                out = self.diffusion_model(
                    x, t, context=cc, **kwargs
...
class FrameInterpPredLatentDiffusion(LatentDiffusion):
    def __init__(
        ...
        n_prevs=[1],
        mergenoisy=True,
        mask_cond_loss=False,
        uncondnoS=False,
        interp_frame_stride=None,
        concat_mask_on_input=True,
        *args,
        **kwargs,
    ):
    ...
    def add_frames_condition(self, input, c):
        ...
        if self.rand_temporal_mask:
            mask = random_temporal_masking(
                input_shape,
                self.p_interp,
                self.p_pred,
                device,
                n_prevs=self.n_prevs,
                interp_frame_stride=self.interp_frame_stride,
            )
        else:
            raise NotImplementedError
        ...
        if self.noisy_cond:
            ...
            s = torch.randint(
                0, self.max_noise_level, (input_shape[0],), device=device
            ).long()
        ...
        if not is_uncond:
            ...
            cond_frames = input[
                :, :, cond_idx, :, :
            ]  # for both prediction & interpolation
            if self.noisy_cond:
        ...
        if "original_cond" in cond:
            ...
            if isinstance(original_cond, torch.Tensor):
                ...
                cond = {
                    **cond,
                    **original_cond,
                }  # original_cond: 'c_crossattn', 'temporal_context'
            else:
                raise NotImplementedError

        if (
            "noisy_cond" in cond and cond["noisy_cond"] is not None
            ...
            if self.p_pred != 0.0 and self.p_interp == 0:  # only prediction
                ...
                if T_cond == 2:
                    ...
                    for i in range(T_cond):
                        x_noisy[:, :, i * self.interp_frame_stride, :, :] = cond[
                            "noisy_cond"
                        ][:, :, i, :, :]

            elif (
                self.p_pred != 0.0 and self.p_interp != 0
            ):  # both interpolatin & prediction
                raise NotImplementedError
